...
- SeedQR export (Standard and Compact SeedQR, as read by SeedSigner, Krux, Sparrow and Passport) shown in a modal; the entropy box has its own copy and plain-text QR buttons. QR codes also for the public values — account xpub, key with origin, multisig xpubs, descriptor and setup file — and a SeedQR for each seed created in the multisig card
